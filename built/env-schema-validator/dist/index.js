...
__export(index_exports, {
  EnvSchemaValidator: () => EnvSchemaValidator,
  envAssert: () => envAssert,
  envSchema: () => envSchema,
...
function fieldToZod(field) {
  ...
  switch (field.type) {
    ...
    case "number": {
      let zod = import_zod.z.coerce.number();
      if (field.min !== void 0) {
    ...
    case "boolean": {
      ...
      schema = import_zod.z.string().refine(
    ...
    case "url": {
      schema = import_zod.z.string().url().transform((val) => {
        try {
          return new URL(val);
        } catch {
    ...
    case "enum": {
      schema = import_zod.z.enum(field.values);
      break;
    }
    default:
...
var EnvSchemaValidator = class {
  ...
  constructor(schema) {
    this.schema = schema;
    this.zodSchema = import_zod.z.object(
      Object.fromEntries(
